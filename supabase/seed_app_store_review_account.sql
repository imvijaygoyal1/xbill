...
begin
  ...
  if reviewer_id is null then
    ...
    values (
      ...
      crypt('<REDACTED_REVIEWER_PASSWORD>', gen_salt('bf', 10)),
      now(),
    ...
    set instance_id = auth_instance_id,
        encrypted_password = crypt('<REDACTED_REVIEWER_PASSWORD>', gen_salt('bf', 10)),
  ...
  values
    (
      ...
      crypt('<REDACTED_SEED_PASSWORD>', gen_salt('bf', 10)),
      now(),
      '{"provider":"email","providers":["email"]}'::jsonb,

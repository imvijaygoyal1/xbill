...
| ICON-05 | `Assets/AppIcon.appiconset/` (repo root) | **A second, divergent, dead App Icon set.** `project.yml:41` bundles only `xBill/**/*.xcassets`, so this copy ships nowhere — but all 15 PNGs **differ** from the shipping set, and this copy has **corner radii baked into the artwork**, which iOS would mask a second time. It also holds two files the real set lacks (`Icon-20@1x`, `Icon-40@1x`). An edit here changes nothing and looks like it worked. | ⬜ Open | Delete `Assets/`. While there: the shipping set is the legacy 15-size list; since Xcode 14 a single 1024 `universal` entry is enough, and collapsing it removes 14 files that can drift. |

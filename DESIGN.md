...
- Notifications was sharpened in place: the legacy `List`/local notification row and duplicated empty-state artwork were replaced with `XBillScreenHeader`, `XBillScreenContainer`, `XBillScrollView`, `XBillEmptyState`, `XBillSectionHeader`, and reusable `XBillNotificationRow`; unread grouping, mark-all-read, per-alert detail viewing, context-menu and swipe read/unread/delete actions, refresh, loading, empty state, and notification store behavior are preserved. Opening the tab does not automatically clear unread alerts.
- Profile was sharpened in place: the legacy `List`/local settings rows were replaced with a stable normal `ScrollView` inside `XBillScreenBackground`, `XBillScreenHeader`, compact enhanced `XBillProfileCard`, `XBillStatsCard`, `XBillFormSection`, `XBillPaymentHandleRow`, and `XBillSettingsRow`; edit profile, QR, payment handles, notification toggles, app lock, sign out, delete account, legal links, refresh, and stats behavior are preserved.
...
Use `XBillScreenHeader` with title "Notifications", `XBillEmptyState` with a single illustration card when empty, and `XBillNotificationRow` for populated activity. Rows must be actionable: tap opens detail and marks only that alert read, leading swipe/context menu toggles read/unread, and trailing swipe/context menu deletes. Preserve grouped dates, badges, mark-all-read behavior, refresh/loading behavior, native tab-bar behavior, and notification store integration. Badge counts must come from per-item read state and should refresh on launch/foreground activation; do not clear badges just because the user opened the tab.

### Profile

Use `XBillScreenBackground` with a normal child-sized `ScrollView`, `XBillScreenHeader`, `XBillProfileCard`, `XBillStatsCard`, `XBillFormSection`, `XBillPaymentHandleRow`, and `XBillSettingsRow`. Preserve native tab-bar behavior, sign out, account deletion, notification toggles, app lock, legal links, QR flow, edit profile flow, payment handles, refresh, and profile stats. Email should stay single-line with middle truncation, and Edit must remain a non-wrapping pill.

...
- **`PaymentDiagnostics` (DEBUG-only instrumentation) added** — `xBill/Core/PaymentDiagnostics.swift`. Three sinks: `os_log`, `print` (captured by `devicectl device process launch --console`), and a persisted `Documents/payment-diagnostics.log` retrievable with `devicectl device copy from --domain-type appDataContainer --domain-identifier com.vijaygoyal.xbill --source Documents/payment-diagnostics.log`. The whole implementation is inside `#if DEBUG` and compiles out of Release (Release build verified). The highest-value piece is the **alert-presentation chokepoint** in `View.errorAlert(item:)` / `errorAlert(error:)` in `Extensions.swift`, which records every alert actually shown with its call site — that is what proved xBill was raising no alert at all.

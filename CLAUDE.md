...
- **Auth:** Email/password + Sign In with Apple. **Email confirmation is OFF** — verified 2026-08-04: all 6 `auth.users` are `email_confirmed_at` set with `confirmation_sent_at` NULL, i.e. auto-confirmed at signup and no confirmation mail ever sent. This file previously claimed it was ON, which sends anyone debugging signup hunting a delivery problem that does not exist. Note the consequence: email is the identity key for friend discovery (`lookup_profiles_by_email`, `search_profiles`) and group invites, so an unverified address can be registered by someone who does not own it

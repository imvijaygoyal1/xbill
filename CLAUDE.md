...
- `xBill/Services/AuthService.swift` — `signUpWithEmail`, `signInWithEmail`, `signInWithApple` (CryptoKit SHA256 nonce), `signOut`, `fetchProfile`, `currentUser()`; `sendPasswordReset` includes `redirectTo: URL(string: "xbill://reset")!` so deep link triggers `.passwordRecovery` event; all table refs use `"profiles"` (not `"users"`)
...
- `xBill/Views/Expenses/ReceiptReviewView.swift` — item review, member chip assignment, tax/tip display, per-person totals; "Use These Splits" calls `onConfirmed` then dismisses back to `AddExpenseView`; currency from `vm.scannedReceipt?.currency ?? "USD"` (no more hardcoded "USD"); file-private `ItemRow` subview with inline price `TextField` (edits `unitPrice` via `vm.updateUnitPrice`); "All" chip at front of member assignment row calls `vm.toggleAssignAll(to:)`; unassigned-items warning banner shown when members exist but any item has empty `assignedUserIDs`; delta-containing validation warning displayed as-is from VisionService
...
`ProfileViewModel.deleteAccount()` deletes the profile row then calls the `delete-account` Edge Function (`supabase/functions/delete-account/index.ts`) which uses the service role key server-side to delete the auth user. Deploy with `supabase functions deploy delete-account`.

...
- `xBill/Services/VisionService.swift` — Two-tier receipt parsing + 5 quality improvements (2026-04-30). Public API: `scanReceipt(from:)` (single image) and `scanMultiPage(from:)` (all pages). Returns `ScanResult(receipt:confidence:tier:validationWarning:suggestedCategory:)`. **Gap 2 — Quality Gate**: `checkImageQuality` throws user-facing `AppError.validationFailed` for too-dark (<12% avg luminance via `CIAreaAverage`), too-blurry (<2% edge energy via `CIPhotoEffectNoir`+`CIEdges`), or no-text (`VNDetectTextRectanglesRequest`). **Gap 3 — OCR Config**: `customWords` (27 receipt domain terms), `minimumTextHeight=0.015` (filters footnotes), `recognitionLanguages` = device preferred + "en-US" fallback, `topCandidates(3)` per observation. **Gap 4 — Date Extraction**: `extractTransactionDate(from:)` uses `NSDataDetector` on full OCR text; filters future (>tomorrow) and stale (>5 years) dates; closest-to-today wins; stored in `Receipt.transactionDate`. **Gap 5 — NL Framework**: `detectLanguage(from:)` via `NLLanguageRecognizer` → BCP-47 tag passed to Apple Intelligence; `suggestCategory(merchant:items:)` keyword-matches 7 categories in `ScanResult.suggestedCategory`. **Gap 6 — Multi-Page**: `scanMultiPage` runs OCR on each page, offsets Y by `pageIndex/pageCount`, adjusts `groupIntoRows` threshold proportionally; all pages combined before tier selection.

...
- **UI-test credential fallback cleanup — 2026-07-12** — Removed stale local `UITestCredentials.plist` fallback reads from `GroupFlowUITests` and `RegressionUITests`. Authenticated UI tests now rely on `XBILL_TEST_EMAIL` / `XBILL_TEST_PASSWORD` from the environment or test-bundle metadata only; `project.yml` continues to exclude the ignored local plist so credentials are not committed or bundled accidentally.
...
- **M-62** — `GroupFlowUITests`: Added `test000SignInWithEnvironmentCredentials()` and helpers to sign in through the email auth UI when `XBILL_TEST_EMAIL` / `XBILL_TEST_PASSWORD` are supplied by environment or test-bundle metadata. No credentials are stored in source; local `UITestCredentials.plist` fallback was removed on 2026-07-12 after XcodeGen excluded that ignored file from the test target.
